    ...
    var admin = {
        ...
        msg: {
            ...
            error: function (msg, callback) {
                ...
                var index = layer.msg(msg, {icon: 2, shade: admin.config.shade, scrollbar: false, time: 1000, shadeClose: true}, callback);
                return index;
            },
            // 警告消息框
        ...
        table: {
                ...
                $.each(data, function (i, v) {
                    if (v === 'refresh') {
                        ...
                        if (admin.checkAuth('add', elem)) {
                            title = init.add_title ? init.add_title : '添加'
                            toolbarHtml += '<button class="layui-btn layui-btn-normal layui-btn-sm" data-open="' + init.add_url + '" data-title="'+title+'"><i class="fa fa-plus"></i> '+title+'</button>\n';
                        }
                    } else if (v === 'delete') {
            ...
            buildOperatHtml: function (data, operat) {
                var html = '';
                operat.class = operat.class || '';
                operat.style = operat.style || '';
                ...
                if (typeof formatOperat.style == "function") {
                    formatOperat.style = formatOperat.style !== '' ? 'style="' + formatOperat.style(data) + '" ' : '';
                } else {
                    formatOperat.style = formatOperat.style !== '' ? 'style="' + formatOperat.style + '" ' : '';
                }
                ...
                if (operat.method === 'open') {
                    ...
                    formatOperat.method = formatOperat.method !== '' ? 'data-blank="' + operat.url + '" data-title="' + formatOperat.title + '" ' : '';
                ...
                formatOperat.width = (operat.width === undefined) ? '' : 'data-width="' + operat.width + '"'
                formatOperat.height = (operat.height === undefined) ? '' : 'data-height="' + operat.height + '"'
                
                html = '<a ' + formatOperat.class + formatOperat.style + formatOperat.method
                    + formatOperat.width + formatOperat.height + formatOperat.extend + '>'
                    + formatOperat.icon + formatOperat.text
                    + '</a>';
                ...
                $.each(option.operat, function (i, item) {
                    if (typeof item === 'string') {
                        switch (item) {
                            case 'edit':
                                var operat = {
                                    ...
                                    width: option.init.edit_width || undefined,
                                    height: option.init.edit_height || undefined,
                                    extend: ""
                                };
                                operat.url = admin.table.toolSpliceUrl(operat.url, operat.field, data);
                                if (admin.checkAuth(operat.auth, elem)) {
                                    html += admin.table.buildOperatHtml(data, operat);
                                }
                                break;
                            case 'delete':
                                var operat = {
                        ...
                        $.each(item, function (i, operat) {
                            if (Array.isArray(operat.class)) {
                                operat.class = operat.class[data[operat.type_field]];
                            }
                            else {
                                operat.class = operat.class || '';
                            }
                            if (Array.isArray(operat.style)) {
                                operat.style = operat.style[data[operat.type_field]];
                            }
                            else {
                                operat.style = operat.style || '';
                            }
                            ...
                            if (Array.isArray(operat.title)) {
                                operat.title = operat.title[data[operat.type_field]];
                            }
                            else {
                                operat.title = operat.title || operat.text;
                            }
                            if (Array.isArray(operat.text)) {
                                operat.text = operat.text[data[operat.type_field]];
                            }
                            else {
                                operat.text = operat.text || operat.title;
                            }
                            operat.extend = operat.extend || '';
                            operat.width = operat.width || undefined;
                            operat.height = operat.height || undefined;
                            ...
                            operat.url = admin.table.toolSpliceUrl(operat.url, operat.field, data);
                            if (admin.checkAuth(operat.auth, elem)) {
                                if (typeof operat.exist_field === 'function') { //exist_field 检查这个按钮是否需要存在
                                    if (operat.exist_field(data) == true) { //exist_field 是函数 则调用函数检查
                                        html += admin.table.buildOperatHtml(data, operat);
                                    }
                                } else if (operat.exist_field == 1 || data[operat.exist_field]) { //或者通过data数据索引检查
                                    html += admin.table.buildOperatHtml(data, operat);
                                }                                
                            }
                        });
        ...
        listen: function (preposeCallback, ok, no, ex) {
            ...
            // 监听表单提交事件 
            admin.api.formRadio(preposeCallback);  //radio 是 render 中动态渲染出来的，所以要放在它后面调用
            ...
            // 监听弹出层的打开
            $('body').on('click', '[data-blank]', function () {
        ...
        api: {
            ...
            formRadio: function (preposeCallback) {
                if (typeof preposeCallback === 'object') {
                    preposeCallback = preposeCallback['radio']
                }
                if (typeof preposeCallback === 'function')
                {
                    var radioList = document.querySelectorAll(".layui-form-radio");
                    if (radioList.length > 0) {
                        $.each(radioList, function (i, v) {
                            var filter = $(this).attr('lay-filter')
                            form.on('radio', function (data) {
                                preposeCallback(data, filter, admin)
                            })
                        })
                    }
                }
            ...
            formSubmit: function (preposeCallback, ok, no, ex) {
                if (typeof preposeCallback === 'object') {
                    preposeCallback = preposeCallback['submit']
                }
                ...
                if (formList.length > 0) {
                    $.each(formList, function (i, v) {
                        ...
                        form.on('submit(' + filter + ')', function (data) {
                            ...
                            if (typeof preposeCallback === 'function') {
                                dataField = preposeCallback(dataField, filter, admin)
                                if (dataField === true)
                                {
                                    return false;
                                }
